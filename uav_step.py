...
from Quadrotor.quadrotor import State
import numpy as np
import random
...
def uav_step(pos, attitude, waypoints, ax, state=None, STEP=False):
    ...
    if STEP == False:
        Quad = Quadrotor(pos, attitude,USE_STEP=False)
    else:
        Quad = Quadrotor(pos, attitude,state,USE_STEP=True)
...
if FLIGHT_HORIZENTAL == False:
    ...
    goal = np.vstack((goal, tem_point))
waypoints = goal
#print(f"waypoints is {waypoints}, shape is {waypoints.shape}")

pos = None
attitude = [0,0,0]
state = uav_step(pos, attitude, waypoints, ax, STEP=False)
#print(state)
...
for i in range(8):      
    ...
    if FLIGHT_HORIZENTAL == False:
        for i in range(5):
            tem_point = np.array([random.uniform(0,1), random.uniform(0,1), random.uniform(0,1)])
            goal = np.vstack((waypoints[-1], tem_point))
    else:
        for i in range(5):
            tem_point = np.array([random.uniform(0,1), random.uniform(0,1)])
            tem_point = np.hstack((tem_point,1))
    ...
    attitude = None
    state = uav_step(pos, attitude, waypoints, ax, state, STEP=True)
